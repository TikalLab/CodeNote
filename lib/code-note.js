'use babel';

import CodeNoteView from './code-note-view';
import SelectBoardView from './select-board';
import { CompositeDisposable } from 'atom';
const request = require('request');
const inspect = require('util-inspect');
const modalController = require('./modalController')
const _ = require('underscore')
const trelloToken = 'b0215d3c3271ffbafb9aeec69f5245fba803926b3ef49289b0dfff3928713c79';
const trelloKey = '38a81889c8c5c443bf606d7af2db28fd';
const exampleTerlloAPICall = 'https://api.trello.com/1/members/me/boards?key=38a81889c8c5c443bf606d7af2db28fd&token=b0215d3c3271ffbafb9aeec69f5245fba803926b3ef49289b0dfff3928713c79';

export default {

  codeNoteView: null,
  modalPanel: null,
  subscriptions: null,
  selectBoardsView: null,
  modalController: null,

  activate(state) {
    this.codeNoteView = new CodeNoteView(state.codeNoteViewState);
    this.modalPanel = atom.workspace.addModalPanel({
      item: this.codeNoteView.getElement(),
      visible: false
    });

    // Events subscribed to in atom's system can be easily cleaned up with a CompositeDisposable
    this.subscriptions = new CompositeDisposable();

    // Register command that toggles this view
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'code-note:toggle': () => this.toggle()
    }));
  },

  deactivate() {
    this.modalPanel.destroy();
    this.subscriptions.dispose();
    this.codeNoteView.destroy();
  },

  serialize() {
    return {
      codeNoteViewState: this.codeNoteView.serialize()
    };
  },

  toggle() {
    request(exampleTerlloAPICall,function(error,response,body){
      if(error){

      }else if(response.statusCode > 300){

      }else{
        var boards = JSON.parse(body);
        var boardNames = _.map(boards,function(board){
          return board.name;
        })

        modalController.selectBoard(boardNames);

        // this.selectBoardsView = new SelectBoardView(this,boardNames);

        // var selectBoardModalPanel = atom.workspace.addModalPanel({
        //   item: selectBoardsView.getElement(),
        //   visible: true
        // });

        console.log('found boards: %s',inspect(boardNames))
        console.log('CodeNote was toggled!');
        return (
          this.modalPanel.isVisible() ?
          this.modalPanel.hide() :
          this.modalPanel.show()
        );
      }
    });

  },

  boardSelected(boardName){
    console.log('user selected the board: %s',boardName)
    this.selectBoardsView.cancel();
  }

};
